...
class NegativeSamplePool:
    ...
    def update(self, text_cls_tokens, labels):
        ...
        for i in range(batch_size):
            ...
            if label_key not in self.pool:
                self.pool[label_key] = torch.empty(
                    (0, sample_token.size(0)), device=self.device
                )

            # 检查是否已存在相同token
            if not self._is_token_duplicate(sample_token, self.pool[label_key]):
                # 添加新token到对应的标签组合池中
                self.pool[label_key] = torch.cat(
                    [self.pool[label_key], sample_token.unsqueeze(0)], dim=0
                )
            else:
                # 记录重复token
                batch_duplicates += 1
    ...
    def load(self, load_path):
        ...
        for label_key, tokens_array in loaded_data["pool"].items():
            if tokens_array.size > 0:  # 确保非空
                tokens_tensor = torch.tensor(
                    tokens_array, dtype=torch.float32, device=self.device
                )
                ...
                # 创建空tensor
                self.pool[label_key] = torch.empty(
                    (0, tokens_array.shape[1] if tokens_array.shape else 0),
                    device=self.device,
                )
                num_samples = 0
        ...
        for label_key, label_array in loaded_data["label_vectors"].items():
            self.label_vectors[label_key] = torch.tensor(
                label_array, dtype=torch.float32, device=self.device
            )
    ...
    def _find_k_smallest_similarities(self, target_label, k=10):
        ...
        similarities = F.cosine_similarity(
            self.vectors_tensor, target_label.unsqueeze(0), dim=1
        )
    ...
    def get_negative_samples_batch(self, target_labels, k=10):
        ...
        # 计算每个标签的可用样本数，避免重复计算
        available_samples = {
            label_key: samples.size(0) for label_key, samples in self.pool.items()
        }
        ...
        for i in range(batch_size):
            ...
            # 找到相似度最小的k个标签
            smallest_similarities = self._find_k_smallest_similarities(
                target_label, k=min(k, len(self.label_vectors))
            )
            ...
            for label_key, _ in smallest_similarities:
                ...
                if current_size <= samples_needed:
                    ...
                    # 随机选择部分样本
                    selected_indices = torch.randperm(current_size, device=self.device)[
                        :samples_needed
                    ]
