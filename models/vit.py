...
class RegionClassifier(nn.Module):
    ...
    def forward(self, hidden_states, region_detected=None):
        ...
        # 2. 区域预测 (使用其他tokens)
        region_features = hidden_states[
            :, 1:, :
        ]  # [batch_size, num_regions, hidden_size]
        region_preds = self.region_classifier(
            region_features
        )  # [batch_size, num_regions, num_diseases]
...
class MedicalVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.encoder.layer):
            ...
            if i % 2 == 0:
                # 通过分类器
                region_preds, global_preds = self.classifiers[i](
                    hidden_states, region_detected
                )
        ...
        if image_labels is not None:
            ...
            for i, (region_preds, image_preds) in enumerate(
                zip(all_region_preds, all_image_preds)
            ):
                layer_loss, global_loss, region_loss = self.classifiers[
                    i * 2
                ].compute_loss(  # 注意这里使用i*2因为是偶数层
                    region_preds,
                    image_preds,
                    region_labels,
                    image_labels,
