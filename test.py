...
def analyze_trace_file(trace_file_path):
    ...
    # 1. 按模块统计总耗时
    module_summary = (
        df.groupby("module")["duration_ms"]
        .agg(["sum", "mean", "count"])
        .sort_values("sum", ascending=False)
    )
    module_summary["percentage"] = (
        module_summary["sum"] / module_summary["sum"].sum() * 100
    )

    # 2. 区分CPU和CUDA操作
    cat_summary = (
        df.groupby(["module", "category"])["duration_ms"].sum().unstack().fillna(0)
    )
    ...
    actual_time = (
        df["timestamp"].max()
        - df["timestamp"].min()
        + df.loc[df["timestamp"].idxmax(), "duration_ms"]
    )
    ...
    memory_intensive_ops = (
        df[df["name"].str.contains("allocate|free|to|copy", case=False)]
...
def extract_module_name(name):
    ...
    layer_match = re.search(
        r"(conv\d+|linear\d*|bn\d+|relu|pool\d*|dropout)", name.lower()
    )
    if layer_match:
...
def recommend_optimizations(analysis_results):
    ...
    for module, row in top_time_modules.iterrows():
        recommendations.append(
            f"- **{module}**: 占总时间的 {row['percentage']:.1f}%, 被调用 {row['count']} 次, 平均耗时 {row['mean']:.2f}ms"
        )
    ...
    if not memory_intensive_ops.empty:
        ...
        for op, row in memory_ops.iterrows():
            recommendations.append(
                f"- **{op}**: 总耗时 {row['sum']:.2f}ms, 被调用 {row['count']} 次"
            )

        # 检查是否有大量to/copy操作
        if any("to" in op.lower() or "copy" in op.lower() for op in memory_ops.index):
            recommendations.append(
                "- **建议**: 减少设备间数据传输，预先将数据移至目标设备，避免频繁的CPU-GPU数据移动"
            )

    # 4. 模型架构优化建议
    model_ops = [
        op
        for op in top_time_modules.index
        if "conv" in op.lower() or "linear" in op.lower() or "matmul" in op.lower()
    ]
    if model_ops:
        recommendations.append("\n## 模型架构优化")
    ...
    if "data_preparation" in module_summary.index:
        ...
        if data_prep["percentage"] > 10:  # 如果数据准备占比超过10%
            recommendations.append("\n## 数据加载优化")
            recommendations.append(
                f"- 数据准备占总时间的 {data_prep['percentage']:.1f}%"
            )
    ...
    recommendations.append("\n## 总体优化方向")
    recommendations.append(
        "1. **减少主机-设备内存传输**: 避免频繁使用`.item()`, `.cpu()`, `.numpy()`等导致同步的操作"
    )
    recommendations.append(
        "2. **优化批处理大小**: 找到最佳的batch_size平衡内存使用和计算效率"
    )
    recommendations.append(
        "3. **使用混合精度训练**: 启用`torch.amp`减少内存使用和加速计算"
    )
    recommendations.append(
        "4. **检查梯度计算**: 使用`torch.no_grad()`避免不必要的梯度计算"
    )
