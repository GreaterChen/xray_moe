...
def compute_map(predictions, ground_truths, iou_threshold=0.5):
    ...
    for i in range(num_preds):
        ...
        if ious.max() >= iou_threshold:
            ...
            # 检查该真值是否已被检测到
            already_matched = (tp.sum() > 0) and (
                (all_gt_image_ids == pred_image_id) & (all_gt_labels == pred_label)
            ).sum() > 0
...
def test_detection_model(model, test_loader, device, output_dir="./test_results"):
    ...
    class_aps = [
        (class_id, metrics["AP"]) for class_id, metrics in per_class_metrics.items()
    ]
